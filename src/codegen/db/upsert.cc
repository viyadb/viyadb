...
void ValueParser::Visit(const db::TimeDimension* dimension) {
  ...
  if (is_num_input) {
    ...
    if (!dimension->rollup_rules().empty() || !dimension->granularity().empty()) {
      code_<<" ctx->time"<<dim_idx<<".set_ts(ctx->upsert_dims._"<<dim_idx<<");\n";
    }
  } else {
    code_<<" ctx->time"<<dim_idx<<".parse(\""<<format<<"\", values["<<tuple_idx_map_[value_idx_]<<"]);\n";
    if (!dimension->rollup_rules().empty()) {
      code_<<" ctx->upsert_dims._"<<dim_idx<<" = ctx->time"<<dim_idx<<".get_ts();\n";
    }
  }

  if (!dimension->rollup_rules().empty()) {
    TimestampRollup ts_rollup(dimension, "ctx->upsert_dims._" + dim_idx, "ctx->");
    code_<<ts_rollup.GenerateCode();
  }
  else if (!dimension->granularity().empty()) {
    code_<<" ctx->time"<<dim_idx<<".trunc<static_cast<viya::util::TimeUnit>("
...
Code UpsertGenerator::UpsertContextCode() const {
  ...
  code<<" std::unordered_map<Dimensions,size_t,DimensionsHasher> tuple_offsets;\n";
  if (AddOptimize()) {
  ...
  for (auto* dimension : table.dimensions()) {
    ...
    if (dimension->dim_type() == db::Dimension::DimType::STRING) {
      ...
      code<<" db::DictImpl<"<<dimension->num_type().cpp_type()<<">* v2c"<<dim_idx<<";\n";
    }
  }
  ...
  if (desc_.has_partition_filter()) {
    code<<" bool partition_values["<<desc_.partition_filter().total_partitions()<<"] = {false};\n";
  }
...
Code UpsertGenerator::SetupFunctionCode() const {
  ...
  for (auto* dimension : table.dimensions()) {
    if (dimension->dim_type() == db::Dimension::DimType::STRING) {
      auto dim_idx = std::to_string(dimension->index());
      code<<" ctx->dict"<<dim_idx<<" = static_cast<const db::StrDimension*>(ctx->table->dimension("<<dim_idx<<"))->dict();\n";
      code<<" ctx->v2c"<<dim_idx<<" = reinterpret_cast<db::DictImpl<"<<dimension->num_type().cpp_type()
        <<">*>(ctx->dict"<<dim_idx<<"->v2c());\n";
    }
  }
  if (desc_.has_partition_filter()) {
    code<<" for(auto& v : desc.partition_filter().values()) ctx->partition_values[v] = true;\n";
  }
  code<<" return static_cast<void*>(ctx);\n";
  code<<"}\n";

  code<<"extern \"C\" void viya_upsert_before(void* ctx_ptr) __attribute__((__visibility__(\"default\")));\n";
  ...
  code<<"extern \"C\" db::UpsertStats viya_upsert_after(void* ctx_ptr) __attribute__((__visibility__(\"default\")));\n";
...
Code UpsertGenerator::OptimizeFunctionCode() const {
  ...
  for (auto& guard : table.cardinality_guards()) {
    auto dim_idx = std::to_string(guard.dim()->index());
    code<<" for (auto it = ctx->card_stats"<<dim_idx<<".begin(); it != ctx->card_stats"<<dim_idx<<".end(); ++it) {\n";
...
Code UpsertGenerator::CardinalityProtection() const {
  ...
  for (auto& guard : table.cardinality_guards()) {
    ...
    for (auto per_dim : guard.dimensions()) {
      ...
      code<<" ctx->card_dim_key"<<dim_idx<<"._"<<per_dim_idx<<" = ctx->upsert_dims._"<<per_dim_idx<<";\n";
    }
    code<<" auto it = ctx->card_stats"<<dim_idx<<".find(ctx->card_dim_key"<<dim_idx<<");\n";
    ...
    code<<"  ctx->card_stats"<<dim_idx<<".emplace(ctx->card_dim_key"<<dim_idx<<", std::move(bitset));\n";
    code<<" } else {\n";
...
Code UpsertGenerator::PartitionFilter() const {
  ...
  if (desc_.has_partition_filter()) {
    ...
    code<<" if(!ctx->partition_values[hash % "<<std::to_string(part_filter.total_partitions())<<"]) return;\n";
    code<<"}\n";
  }
  return code;
}
...
Code UpsertGenerator::GenerateCode() const {
  ...
  code<<"extern \"C\" void viya_upsert_do(void* ctx_ptr, std::vector<std::string>& values) __attribute__((__visibility__(\"default\")));\n";
  code<<"extern \"C\" void viya_upsert_do(void* ctx_ptr, std::vector<std::string>& values) {\n";
  code<<" UpsertContext* ctx = static_cast<UpsertContext*>(ctx_ptr);\n";
  ...
  code<<"  ctx->tuple_offsets.emplace(ctx->upsert_dims, segment_idx * "<<segment_size<<" + tuple_idx);\n";
  code<<" }\n";
  ...
  for (auto* metric : table.metrics()) {
    if (metric->agg_type() == db::Metric::AggregationType::BITSET) {
      auto metric_idx = std::to_string(metric->index());
      code<<" ctx->upsert_metrics._"<<metric_idx<<" = ctx->empty_bitset"<<metric_idx<<";\n";
    }
  }
  code<<"}\n";
  return code;
