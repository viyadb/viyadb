...
template <int SizeBytes> class Bitset {
  ...
  using NumType =
      typename std::conditional<SizeBytes == 8, uint64_t, uint32_t>::type;
...
public:
  ...
  /**
   * Add element if it doesn't exist yet. Otherwise, cached cardinality will be
   * wrong!
   */
  ...
  bool contains(NumType num) { return roaring_.contains(num); }
  ...
  void optimize() { roaring_.runOptimize(); }
