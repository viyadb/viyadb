...
uint32_t Duration::add_to(uint32_t timestamp, int sign) const {
  ...
  switch (time_unit_) {
  case TimeUnit::YEAR:
    tm.tm_year += sign * count_;
    break;
  case TimeUnit::MONTH:
    tm.tm_mon += sign * count_;
    break;
  case TimeUnit::WEEK:
    tm.tm_mday += 7 * sign * count_;
    break;
  case TimeUnit::DAY:
    tm.tm_mday += sign * count_;
    break;
  case TimeUnit::HOUR:
    tm.tm_hour += sign * count_;
    break;
  case TimeUnit::MINUTE:
    tm.tm_min += sign * count_;
    break;
  case TimeUnit::SECOND:
    tm.tm_sec += sign * count_;
    break;
  default:
