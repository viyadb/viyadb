...
Session::Session(const Consul &consul, const std::string &name,
  ...
  repeat_ = std::make_unique<util::Repeat>((ttl_sec_ * 2000L) / 3,
                                           [this]() { Renew(); });
}
...
void Session::Create() {
  json data = {{"Name", name_},
               {"Checks", json::array()},
               {"LockDelay", "0s"},
               {"Behavior", "delete"},
               {"TTL", std::to_string(ttl_sec_) + "s"}};

  DLOG(INFO) << "Creating new session '" << name_ << "' with TTL=" << ttl_sec_
             << "sec";
  auto r = cpr::Put(
      cpr::Url{consul_.url() + "/v1/session/create"}, cpr::Body{data.dump()},
      cpr::Header{{"Content-Type", "application/json"}}, cpr::Timeout{3000L});
...
void Session::Renew() {
  ...
  auto r = cpr::Put(cpr::Url{consul_.url() + "/v1/session/renew/" + id_},
                    cpr::Timeout{3000L});

  if (r.status_code == 0) {
    LOG(WARNING) << "Can't contact Consul at: " << consul_.url()
                 << " (host is unreachable)";
...
bool Session::EphemeralKey(const std::string &key,
                           const std::string &value) const {
  std::string target_key = consul_.prefix() + "/" + key;
  DLOG(INFO) << "Acquiring lock on key '" << target_key << "' for session '"
             << id_;

  auto r = cpr::Put(cpr::Url{consul_.url() + "/v1/kv/" + target_key},
                    cpr::Body{value}, cpr::Parameters{{"acquire", id_}},
