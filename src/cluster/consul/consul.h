...
public:
  ...
  std::unique_ptr<Service> RegisterService(const std::string &name,
                                           uint16_t port, uint32_t ttl_sec = 10,
                                           bool auto_hc = true) const;

  std::unique_ptr<LeaderElector> ElectLeader(const Session &session,
                                             const std::string &key) const;

  std::unique_ptr<Watch> WatchKey(const std::string &key,
