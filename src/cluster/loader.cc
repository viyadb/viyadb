...
Loader::Loader(const Controller &controller, const std::string &load_prefix)
    : controller_(controller), load_prefix_(load_prefix), load_pool_(4) {}

json Loader::GetPartitionFilter(const std::string &table_name,
  ...
  return std::move(json{{"total_partitions", partitioning.total()},
...
void Loader::LoadFile(const std::string &file, const std::string &table_name,
                      const TableInfo &table_info,
                      const std::string &worker_id) {
  util::ScopeGuard cleanup = [&file]() { fs::remove(file); };
  json request{{"table", table_name},
...
void Loader::LoadFilesToAll(const std::string &path,
...
void Loader::LoadFileToAll(const std::string &file,
                           const std::string &table_name,
                           const TableInfo &table_info) {
  util::ScopeGuard cleanup = [&file]() { fs::remove(file); };
...
void Loader::SendRequest(const std::string &worker_id, const json &request) {
  ...
  auto r = cpr::Post(cpr::Url{url}, cpr::Body{request.dump()},
