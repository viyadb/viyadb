...
UIntType max_value_to_uint_type(size_t max_value) {
  if (max_value - 1 < UINT8_MAX)  return UIntType(BaseNumType::Size::_1);
  if (max_value - 1 < UINT16_MAX) return UIntType(BaseNumType::Size::_2);
  if (max_value - 1 < UINT32_MAX) return UIntType(BaseNumType::Size::_4);
  return UIntType(BaseNumType::Size::_8);
}

NumericType::Type parse_metric_type(const std::string& type_name) {
  if (type_name == "byte")   { return NumericType::Type::BYTE; }
  if (type_name == "ubyte")  { return NumericType::Type::UBYTE; }
  if (type_name == "short")  { return NumericType::Type::SHORT; }
  if (type_name == "ushort") { return NumericType::Type::USHORT; }
  if (type_name == "int")    { return NumericType::Type::INT; }
...
NumericType::NumericType(const Type type):BaseNumType(numeric_type_to_size(type)),type_(type) {
}
...
const std::string NumericType::cpp_type() const {
  if (type_ == Type::BYTE)   { return "int8_t"; }
  if (type_ == Type::UBYTE)  { return "uint8_t"; }
  if (type_ == Type::SHORT)  { return "int16_t"; }
  if (type_ == Type::USHORT) { return "uint16_t"; }
...
const std::string NumericType::cpp_max_value() const {
  if (type_ == Type::BYTE)   { return "INT8_MAX"; }
  if (type_ == Type::UBYTE)  { return "UINT8_MAX"; }
  if (type_ == Type::SHORT)  { return "INT16_MAX"; }
  if (type_ == Type::USHORT) { return "UINT16_MAX"; }
...
const std::string NumericType::cpp_min_value() const {
  if (type_ == Type::BYTE)   { return "INT8_MIN"; }
  if (type_ == Type::UBYTE)  { return "0U"; }
  if (type_ == Type::SHORT)  { return "INT16_MIN"; }
  if (type_ == Type::USHORT) { return "0U"; }
...
const std::string NumericType::cpp_parse_fn() const {
  switch (type_) {
    case Type::BYTE:   return "std::stoi";
    case Type::UBYTE:  return "std::stoul";
    case Type::SHORT:  return "std::stoi";
    case Type::USHORT: return "std::stoul";
    case Type::INT:    return "std::stoi";
...
const AnyNum NumericType::Parse(const std::string& value) const {
  switch (type_) {
    case Type::BYTE:   return AnyNum((int8_t)std::stoi(value));
    case Type::UBYTE:  return AnyNum((uint8_t)std::stoul(value));
    case Type::SHORT:  return AnyNum((int16_t)std::stoi(value));
    case Type::USHORT: return AnyNum((uint16_t)std::stoul(value));
    case Type::INT:    return AnyNum((int32_t)std::stoi(value));
...
NumericType parse_value_metric_type(const util::Config& config, Metric::AggregationType agg_type) {
...
StrDimension::StrDimension(const util::Config& config, size_t index, Dictionaries& dicts)
  ...
  num_type_(max_value_to_uint_type(cardinality_ = config.num("cardinality", UINT32_MAX))),
...
NumericType parse_num_dimension_type(const util::Config& config) {
  std::string type = config.str("type");
  if (type == "numeric") {
    LOG(WARNING)<<"numeric dimension type is deprecated. Please use specific types: byte, ubyte, short, etc.";
    auto count_type = max_value_to_uint_type(config.num("max", UINT32_MAX));
    if (count_type.size() == BaseNumType::Size::_8) {
      return NumericType(NumericType::Type::ULONG);
    }
...
NumDimension::NumDimension(const util::Config& config, size_t index)
  :Dimension(config, index, Dimension::DimType::NUMERIC),num_type_(parse_num_dimension_type(config)) {
}
...
TimeDimension::TimeDimension(const util::Config& config, size_t index, bool micro_precision)
  ...
  num_type_(UIntType(micro_precision ? BaseNumType::Size::_8 : BaseNumType::Size::_4)),
...
BoolDimension::BoolDimension(const util::Config& config, size_t index)
  :Dimension(config, index, Dimension::DimType::BOOLEAN),num_type_(BaseNumType::Size::_1) {
}
